...
#C:\Users\igorz\Desktop\IGOR ZC\Codigo_Dashboard

# Configurações do Firebase
FIREBASE_HOST = st.secrets["FIREBASE_HOST"]
FIREBASE_AUTH = st.secrets["FIREBASE_AUTH"]
...
with area_fundo.container():
    ...
    col1.metric("☀️ Luminosidade", f"{luminosidade} lx")
    col2.metric("🌡️ Temperatura", f"{temperatura} °C")
    col3.metric("💧 Umidade", f"{umidade} %")
    col4.metric("🌱 Solo", f"{solo} %")

    st.line_chart(df[["Luminosidade (lx)", "Temperatura (°C)", "Umidade (%)", "Umidade do Solo (%)"]])
